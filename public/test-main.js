    ...
    _createClass(Launch, [{
        ...
        value: function createDescription() {
            ...
            if (this.data.details) {
                ...
                _missionMain.appendChild(document.createTextNode("No description available yet."))
            }
            return _missionMain
        }
    }, {
        ...
        value: function createLocation() {
            ...
            _missionLocation.appendChild(document.createTextNode(this.data.launch_site.site_name_long));
        ...
        value: function createCountDown() {
            ...
            if (secondsFromT1ToT2 < 0) {
                ...
                _countdown.appendChild(document.createTextNode("".concat(time.days, " days, ").concat(time.hours, " hours, ").concat(time.minutes, " mins, ").concat(time.seconds, " seconds")))
            }
            return _countdown
        }
    }, {
...
var Page = function() {
    ...
    _createClass(Page, [{
        ...
        value: function loadUpcomingLaunches() {
            var _this2 = this;
            fetch("".concat(API_URL, "spacex/launches/upcoming")).then(function(resp) {
                return resp.json()
            }).then(function(json) {
        ...
        value: function loadImageOfTheDay() {
            var _this3 = this;
            fetch("".concat(API_URL, "Nasa/apod")).then(function(resp) {
                return resp.json()
            }).then(function(json) {
                console.log(json);
        ...
        value: function renderPictureOfTheDay() {
            ...
            document.querySelector(".image-title").textContent = this.state.pictureOfTheDay.title;
            document.querySelector(".copyright").textContent = this.state.pictureOfTheDay.copyright || "no copyright";
            hero.style.backgroundImage = "url(".concat(this.state.pictureOfTheDay.url, ")")
        }
    }, {
        key: "renderUpcomingLaunches",
        value: function renderUpcomingLaunches() {
            var _launch = this.state.launches.upcoming[this.state.launches.currentIndex];
        ...
        value: function goToNextLaunch() {
            ...
            if (this.state.launches.currentIndex + 1 > this.state.launches.upcoming.length) {
        ...
        value: function goToPrevLaunch() {
            ...
            if (this.state.launches.currentIndex < 0) {
                this.state.launches.currentIndex = this.state.launches.upcoming.length - 1
            }
            this.renderUpcomingLaunches()
        }
    }, {
